...
export function SessionCard({ session: s, compact = false }: { session: Session; compact?: boolean }) {
  ...
  return (
    <Link href={`/classes/${s.id}`} className={`card block min-w-0 hover:border-brand/40 transition ${cancelled ? "opacity-60" : ""}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          {!compact && <div className="text-xs text-ink-soft">{fmtDate(s.starts_at)}</div>}
          <div className="flex items-start gap-2">
            <span className="h-2.5 w-2.5 rounded-full shrink-0 mt-[0.45em]" style={{ background: s.class_types.colour }} />
            <h3 className="font-semibold text-ink leading-snug min-w-0 break-words">{s.class_types.name}</h3>
          </div>
